...
const SOURCE_ORIGIN = "https://dich-mit-stich.at";
const SOURCE_INDEX = `${SOURCE_ORIGIN}/tattoo-singles/`;
const OUTPUT_FILE = fileURLToPath(new URL("../data/tattoo-cities-at.json", import.meta.url));
const IMAGE_DIR = fileURLToPath(new URL("../public/cities/at/", import.meta.url));
...
const PIXABAY_CREATORS = {
  wien: "hbieser",
  linz: "Leonhard_Niederwimmer",
  dornbirn: "Pixel-Sepp",
  graz: "Bilderwelt68",
  salzburg: "ChiemSeherin",
  klagenfurt: "RoubeOG",
  villach: "jonathanjudmaier",
  wels: "maxmann",
  "sankt-poelten": "Leonhard_Niederwimmer",
  "wiener-neustadt": "StockSnap",
};
...
function extractCityLinks(indexHtml) {
  const found = new Map();
  const pattern = /<a\b[^>]*href="https:\/\/dich-mit-stich\.at\/tattoo-singles\/([^/"?#]+)\/"[^>]*>([\s\S]*?)<\/a>/gi;
  for (const match of indexHtml.matchAll(pattern)) {
    const slug = match[1];
    if (!found.has(slug)) found.set(slug, stripTags(match[2]));
  }
  if (found.size !== 10) throw new Error(`Expected 10 AT city links, found ${found.size}`);
...
function sanitizeEditorialHtml(contentHtml) {
  return contentHtml
    ...
    .replace(/<h[1-6][^>]*>\s*<img\b[^>]*>\s*<\/h[1-6]>/gi, "")
    .replace(/<h[1-6][^>]*>\s*Andere interessante St(?:ä|&auml;)dte[\s\S]*?(?=<p>\s*<small>\s*Bildquelle:|<hr\b|$)/i, "")
    .replace(/<p>\s*<small>\s*Bildquelle:[\s\S]*?<\/p>\s*&gt;?/i, "")
    .replace(/<hr\b[^>]*>[\s\S]*$/i, "")
    ...
    .replace(/https:\/\/dich-mit-stich\.at\/tattoo-singles\//gi, "/tattoo-singles/")
    .trim();
}
...
async function importCity({ slug }, supportedSlugs) {
  const sourceUrl = `${SOURCE_INDEX}${slug}/`;
  const html = await (await fetchChecked(sourceUrl)).text();
  const contentHtml = html.match(
    /<div class="text-content m-t-64 m-b-64">([\s\S]*?)<\/div>\s*<div class="">\s*<a href="https:\/\/dich-mit-stich\.at\/registration\//i,
  )?.[1];
  ...
  return {
    city: {
      ...
      imageAlt,
      imageAttribution: {
        label: `Foto von ${PIXABAY_CREATORS[slug]} auf Pixabay`,
        sourceUrl: decodeHtmlEntities(imageSourceUrl),
        creator: PIXABAY_CREATORS[slug],
        publisher: "Pixabay",
...
async function main() {
  ...
  const inventory = {
    ...
    overview: {
      title: firstMatch(indexHtml, /<title>([\s\S]*?)<\/title>/i, "overview title", SOURCE_INDEX),
  ...
  console.log(`AT city import complete: ${cityLinks.length} cities, JSON ${jsonChanged ? "updated" : "unchanged"}, ${changedImages} images updated`);
}
