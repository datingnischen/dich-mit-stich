...
test("page paths without a trailing slash redirect to the public path with one", () => {
  ...
  const unprefixed = proxy(new NextRequest("https://dich-mit-stich.vercel.app/magazin/tattoo-ideen"));
  assert.equal(unprefixed.headers.get("location"), "https://dich-mit-stich.vercel.app/magazin/tattoo-ideen/");

  for (const path of ["/at/faq/", "/magazin/", "/", "/at/robots.txt", "/sitemap.xml", "/app-assets/brand/icon.png"]) {
    const response = proxy(new NextRequest(`https://dich-mit-stich.vercel.app${path}`));
